# Raspberry Pi ile Motor Kontrol Serisi
Bu repo, YouTube'da yayınlanan Raspberry Pi + SSH + Motor Kontrol serisinin komutlarını, kodlarını ve devre şemalarını içerir.

## Bölümler
- Bölüm 01: Raspberry Pi OS kurulumu, Wi-Fi ve SSH
- Bölüm 02: Motor sürücü bağlantıları (şema/kablolama)
- Bölüm 03: PWM kodları



# Raspberry Pi Motor Control Series
This repository contains the commands, source codes, and circuit schematics used in the Raspberry Pi + SSH + Motor Control series published on YouTube.

## Episodes
- Episode 01: Raspberry Pi OS installation, Wi-Fi and SSH setup
- Episode 02: Motor driver connections (schematic / wiring)
- Episode 03: PWM control codes


## 🎬  YouTube Videoları

### [Bölüm 1 – OS Kurulumu, WiFi & SSH](https://www.youtube.com/watch?v=nQsEsDezThc)

### [Bölüm 2 - Pi ve L298N Bağlantı & Kablolama](https://www.youtube.com/watch?v=xPfAOE4aU1M)

### Bölüm 3 - Kodlama ve PWM 
